...
export default function LogIn({login}) {
  ...
  return (
    ...
    <h2 className='text-white mt-5 m-0 p-3 bg-dark rounded-3'>Log In</h2>
        <form onSubmit={(event)=>{sedData(event)}} className='p-5 rounded-3 form'>
            ...
            {errorMsg.length>0? errorMsg.map((error,i)=>{
              if(error.message.includes('email')){
                ...
                return <p className='d-block text-danger fw-bold'>{error.message}</p>
              }
          ...
          {errorMsg.length>0? errorMsg.map((error,i)=>{
              if(error.message.includes('password')){
                return <p className='d-block text-danger my-3 fw-bold'>{error.message}</p>
              }
